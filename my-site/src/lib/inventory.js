const code_cards = {
    "promos": {
        "ENSVCHZEXSPC": {
            title: "Charizard ex Super-Premium Collection",
            quantity: 1,
        },
        "ENSVOPNPC": {
            title: "Ogerpon ex Premium Collection",
            quantity: 2,
        },
        "ENSVCCSU24": {
            title: "Collector Chest: Scream Tail, Iron Valiant",
            quantity: 1,
        },
        "ENSVBULEXB": {
            title: "Bloodmoon Ursaluna ex Box",
            quantity: 1,
        },
        "ENSVCZREXB": {
            title: "Cyclizar ex Box",
            quantity: 2,
        },
        "ENSV3BLSPMI": {
            title: "Scarlet & Violet-Obsidian Flames 1pk: Pawmi",
            quantity: 3,
        },
        "ENSV3BLSPWP": {
            title: "Scarlet & Violet-Obsidian Flames 1pk: Paldean Wooper",
            quantity: 3,
        },
        "ENSV7ETB": {
            title: "Scarlet & Violet-Stellar Crown Elite Trainer Box",
            quantity: 4,
        },
        "ENSV7BLSHRS": {
            title: "Scarlet & Violet-Stellar Crown 1pk: Horsea",
            quantity: 2,
        },
        "ENSV7BLSPRT": {
            title: "Scarlet & Violet-Stellar Crown 1pk: Porygon2",
            quantity: 1,
        },
    },
    "sets": {
        "ENSWSH11BST2": {
            title: "Sword & Shield-Lost Origin",
            quantity: 2,
        },
        "ENSWSH12BST2": {
            title: "Sword & Shield-Silver Tempest",
            quantity: 5,
        },
        "ENSV1BST": {
            title: "Scarlet & Violet",
            quantity: 8,
        },
        "ENSV3BST": {
            title: "Scarlet & Violet-Obsidian Flames",
            quantity: 9,
        },
        "ENSV4BST": {
            title: "Scarlet & Violet-Paradox Rift",
            quantity: 5,
        },
        "ENSV5BST": {
            title: "Scarlet & Violet-Temporal Forces",
            quantity: 6,
        },
        "ENSV6BST": {
            title: "Scarlet & Violet-Twilight Masquerade",
            quantity: 6,
        },
        "ENSV7BST": {
            title: "Scarlet & Violet-Stellar Crown",
            quantity: 46,
        },
        "ENSV8BST": {
            title: "Scarlet & Violet-Surging Sparks",
            quantity: 6,
        },
        "ENSV8P5BST": {
            title: "Scarlet & Violet-Prismatic Evolutions",
            quantity: 2,
        },
    },
}